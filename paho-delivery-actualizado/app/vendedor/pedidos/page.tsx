...
import { Tag, CheckCircle2, Truck, MessageCircle, Mail } from "lucide-react";

/**
 * Arma un link de WhatsApp a partir del teléfono que ingresó el
 * comprador. Es una heurística simple para Argentina: si el número no
 * viene ya con código de país (54), se lo agrega junto con el 9 que
 * WhatsApp necesita para celulares argentinos. No es perfecto para
 * todos los formatos que alguien pueda escribir, pero cubre el caso
 * común (número de celular con o sin el 0/15 iniciales).
 */
function linkWhatsapp(telefono: string | null | undefined): string | null {
  if (!telefono) return null;
  const digitos = telefono.replace(/\D/g, "");
  if (!digitos) return null;
  const conCodigoPais = digitos.startsWith("54") ? digitos : `549${digitos}`;
    ...
    <main className="min-h-screen bg-paper-texture">
      ...
      <section className="mx-auto max-w-3xl px-5 py-16">
        ...
        <p className="text-charcoal/60 text-sm mb-10">
          Acá aparecen los pedidos con el pago ya confirmado.
        </p>
        ...
        {cargandoPedidos ? (
          ...
          <div className="ficha bg-white border border-line p-8 text-center text-sm text-charcoal/60">
            Todavía no tenés pedidos con el pago confirmado.
          </div>
        ) : (
              ...
              <div key={p.id} className="ficha bg-white border border-line p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {linkWhatsapp(p.envio?.telefonoContacto) && (
                      <a
                        href={linkWhatsapp(p.envio?.telefonoContacto)!}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-1 inline-flex items-center gap-1 text-xs text-moss hover:underline"
                      >
                        <MessageCircle size={13} />
                        {p.envio?.telefonoContacto}
                      </a>
                    )}
                  </div>
                ...
                <Link
                  href={`/vendedor/pedidos/${p.id}/mensajes`}
                  className="mt-3 ml-2 inline-flex items-center gap-1.5 text-xs text-ink border border-line rounded-stamp px-2.5 py-1.5 hover:border-ink/40 transition-colors"
                >
                  <Mail size={13} />
                  Mensajes
                </Link>
